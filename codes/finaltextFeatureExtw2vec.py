...
for c,i in zip(cap,imgId):
    for sen in c:
        ...
        try:
            ...
            with open("/home/subha/RefExpCVPR2018/penseur/textFeaturescacheword2vec/text_"+i+ ".txt","wb") as fp2:
                pickle.dump(f,fp2)
...
for i,s in enumerate(sentences):
        t= LabeledSentence(words = s.split(),tags=['SENT_%s'%i])
        a.append(t)
vec =[]
...
sent_id=0
for item in model:
    vec.append(model.docvecs["SENT_"+str(sent_id)])
    sent_id += 1

with open("textFeaturesword2vec.txt","wb") as fp:
    pickle.dump(vec,fp)
